...
import { IOTA_MESSENGER_UI } from '@/frontend/lib/product-identity'
import {
  ...
  return (
    <>
      ...
      {(isPrivate || !encrypted) && !encrypted && forcedTransport !== 'mesh' ? (
        <span className="sr-only">Klartext-Online nutzt den IOTA-Pfad.</span>
      ) : null}

      {(isPrivate || !encrypted) && !IOTA_MESSENGER_UI.hideFunk && forcedTransport === 'mesh' && !meshBleConnected && onOpenPartnerSetup ? (
      ...
      {(isPrivate || !encrypted) && !IOTA_MESSENGER_UI.hideFunk && forcedTransport === 'adhoc' && (
