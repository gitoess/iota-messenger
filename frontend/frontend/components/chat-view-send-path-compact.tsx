...
export function ChatViewSendPathCompact(p: ChatViewSendPathCompactProps) {
  ...
  const showTelegramPath = Boolean(onComposerDeliveryChange) && !IOTA_MESSENGER_UI.hideTelegram
  const showFunkPath = !IOTA_MESSENGER_UI.hideFunk
  const showAdhocPath = showAdhocTransport && !isIotaMessengerProduct()
